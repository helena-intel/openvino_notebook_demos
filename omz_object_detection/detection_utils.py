...
from omz_model_api import SSD, YOLO, FaceBoxes, CenterNet, RetinaFace
from omz_model_api.utils import InputTransform
...
def get_model(ie, model, architecture_type, labels, keep_aspect_ratio=False, prob_threshold=0.5):
    input_transform = InputTransform(False, 0, 1)
    if architecture_type == "ssd":
        return SSD(ie, model, labels=labels, input_transform=input_transform,keep_aspect_ratio_resize=keep_aspect_ratio)
    elif architecture_type == "yolo":
        return YOLO(ie, model, labels=labels, input_transform=input_transform,threshold=prob_threshold, keep_aspect_ratio=keep_aspect_ratio)
    elif architecture_type == "faceboxes":
        return FaceBoxes(ie, model,input_transform=input_transform, threshold=prob_threshold)
    elif architecture_type == "centernet":
        return CenterNet(ie, model, labels=labels, input_transform=input_transform,threshold=prob_threshold)
    elif architecture_type == "retina":
        return RetinaFace(ie, model,input_transform=input_transform, threshold=prob_threshold)
    else:
